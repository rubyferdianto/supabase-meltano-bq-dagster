...
def upload_csv_with_complete_workflow(file):
    ...
    try:
        ...
        # Check file size for Supabase storage limits
        file_size_mb = os.path.getsize(file.name) / (1024 * 1024)
        workflow_log.append(f"📏 File size: {file_size_mb:.2f} MB")
        
        # Supabase storage has ~50MB limit, use direct processing for large files
        use_direct_processing = file_size_mb > 45 or len(df) > 500000
        
        if use_direct_processing:
            workflow_log.append("🚀 LARGE FILE DETECTED - Using direct processing (skip storage upload)")
            workflow_log.append("📋 Direct Mode: CSV → Clean → Database (no storage archival)")
            
            # Skip to Step 2 directly with the loaded DataFrame
            import_file_path = f"direct_processing/{unique_filename}"
            archive_file_path = f"direct_processing_archive/{unique_filename}"
            df_from_bucket = df  # Use original DataFrame
            
        else:
            # ==========================================
            # STEP 1: Upload CSV to csv-import folder (Small files only)
            # ==========================================
            ...
            # ==========================================
            # STEP 2: Download and process CSV from storage
            # ==========================================
        ...
        # ==========================================
        # STEP 2: Process CSV data for database import
        # ==========================================
        workflow_log.append(f"\n💾 STEP 2: Processing CSV data for database import...")
        
        if df_from_bucket is not None:
            try:
            ...
            workflow_log.append("❌ Step 2 Skipped: Could not load CSV data")
            database_success = False
            inserted_count = 0
            df_clean = None
        ...
        # ==========================================
        # STEP 3: Archive file (if successful and not direct processing)
        # ==========================================
        if database_success:
            if use_direct_processing:
                workflow_log.append("\n📁 STEP 3: Direct processing mode - no storage archival needed")
                workflow_log.append("✅ Large file processed directly without storage upload")
                
                # ==========================================
                # STEP 4: Cleanup (not needed for direct processing)
                # ==========================================
                workflow_log.append("\n🗑️ STEP 4: Direct processing mode - no cleanup needed")
                workflow_log.append("\n🎉 ALL STEPS COMPLETED SUCCESSFULLY (DIRECT MODE)!")
            ...
            if use_direct_processing:
                workflow_log.append("\n⚠️ STEP 3 & 4 SKIPPED: Database import failed (direct processing mode)")
                workflow_log.append(f"💡 Large file processing failed - check database connection and table structure")
            else:
        ...
        try:
            if use_direct_processing:
                public_url = "Direct processing mode - no storage URL"
            else:
        ...
        # ==========================================
        file_size_mb = file_size_mb if 'file_size_mb' in locals() else len(open(file.name, 'rb').read()) / (1024 * 1024)
